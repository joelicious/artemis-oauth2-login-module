...
public class Oauth2LoginModule implements LoginModule {
	...
	private String userName = "oauth2-token";
	...
	public void initialize(Subject subject, CallbackHandler callbackHandler, Map<String, ?> sharedState,
		...
		if (options.get(OAUTH2_TOKEN) != null) {
			userName = (String) options.get(OAUTH2_TOKEN);
		}
		...
		if (debug) {
			logger.debug("Initialized debug=" + debug + " guestUser=" + userName + " guestGroup=" + roleName + " url="
					+ oauth2URL);
		}
	...
	private boolean determineAuthorization(String url) {
		...
		try {
			...
			while ((inputLine = in.readLine()) != null) {
				...
				// Should not be executed if credentials are bad since 401 Exception is thrown.
